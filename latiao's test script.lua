...
menu.toggle_loop(server, "if you host report ban all moder", {""}, ".", function()
    if NETWORK.NETWORK_IS_HOST() then
        for k, pid in pairs(players.list()) do
            if pid == players.user() then
                goto out
...
menu.toggle_loop(dividends, "jump hack", {"latiaojumphack"}, "latiaojumphack.", function()
    ...
    SET_INT_LOCAL("fm_mission_controller", 1269 + 135, 3)
...
menu.toggle_loop(world, "del cctv_cam", {""}, "", function()
...
menu.action(dividends, "完成别惹德瑞前置", {}, "", function()
...
menu.action(test, "ins tp and back", {"inslatiaotpback"}, "", function()
    local pos = players.get_position(players.user())
...
menu.toggle_loop(server, "nodamage EXPLOSION spamm all player", {"latiaobedsoundforall"}, "latiaobedsoundforall",
    function()
        for k, pid in pairs(players.list()) do
            local pos = players.get_position(pid)
